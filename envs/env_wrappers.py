...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            observe, reward, done, cost = env.step(data)
            [ob, obs_glb] = observe
            ...
            remote.send((ob, obs_glb, reward, done, cost))
        elif cmd == 'reset':
            ob = env.reset()
            remote.send((ob))
        elif cmd == 'render':
...
class GuardSubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        self.waiting = False
        obs, rews, cos, dones = zip(*results)
        return np.stack(obs), np.stack(rews), np.stack(cos), np.stack(dones)
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            ob, obs_glb, reward, done, cost, available_actions = env.step(data)
            if np.all(np.array(done)):
                ob, obs_glb, available_actions = env.reset()

            remote.send((ob, obs_glb, reward, done, cost, available_actions))
        elif cmd == 'reset':
            ob, obs_glb, available_actions = env.reset()
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, share_obs, rews, dones, costs,  available_actions = zip(*results)
...
def chooseworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ob, obs_glb, reward, done, cost, available_actions = env.step(data)
            remote.send((ob, obs_glb, done, cost, available_actions))
        elif cmd == 'reset':
...
class DummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        obs, rews, cos, dones = map(np.array, zip(*results))
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, share_obs, rews, dones, costs, available_actions = map(
        ...
        return obs, share_obs, rews, dones, costs, available_actions
